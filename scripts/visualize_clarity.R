...
df$date <- ifelse(nchar(df$date) == 8, paste0(df$date, " 00:00"), df$date)

df$date <- lubridate::parse_date_time(df$date, orders = c("ymd HM", "ymd"))

df_wet$date <- lubridate::ymd_hms(df_wet$date)

#lets convert the data into daily means
df_wet <- data.frame(period.apply(df_wet, endpoints(df_wet, "day"), colMeans, na.rm = TRUE))

setDT(df_wet, keep.rownames = "date")
df_wet <- df_wet %>%
  mutate(date1 = lubridate::ymd_hms(date))

#move column 'column' to first position
df_wet <- df_wet %>% select(date1, everything())
df_wet <- df_wet[, -2]
colnames(df_wet)[1] <- "date"
...
#lets do the summary
df_sum <- df %>% tidyr::gather(site, pm25, CPC_Tm:Asokwa_Shell_Ksi, na.rm = TRUE)
...
##-------------------#BOXPLOT-----------------------------------
jpeg("figures/Clarity/boxplotalldry.jpeg", units = "cm", width = 40, height = 25, res = 300)
ggplot(df_long, aes(x = fct_inorder(Site), y = pm25, fill = Season)) +
  ...
  geom_boxplot(aes(), width = 0.5, lwd = 1, outlier.shape = NA,
               position = position_dodge(width = 0.5), fill = "yellow") +
...
#--------------------LINEPLOT---------------------------
jpeg("figures/Clarity/lineplot4ksi.jpeg", units = "cm", width = 50, height = 25, res = 300)
ggplot(df, aes(x = date, group = 1)) +
  #geom_line(aes(y = CPC_corrected, colour = "CPC_Tm"), lwd = 1.5, lty = 1) +
  #geom_line(aes(y = St_corrected, colour = "St_Joseph_sch_Acc"),
            #lwd = 1.5, lty = 1) +
  #geom_line(aes(y = Bet_corrected, colour = "Bethlehem_Tm1"),
            #lwd = 1.5, lty = 1) +
  #geom_line(aes(y = Com_corrected, colour = "Comm2_Tm"), lwd = 1.5, lty = 1) +
  #geom_line(aes(y = TMA_corrected, colour = "TMA_Tm"),
            #lwd = 1.5,lty = 1) +
  #geom_line(aes(y = Fre_corrected, colour = "FreeZones_Tm"),
            #lwd = 1.5, lty = 1) +
  #geom_line(aes(y = Val_corrected, colour = "Valco_Rbt_Tm"),
            #lwd = 1.5, lty = 1) +
  #geom_line(aes(y = Chal_corrected, colour = "Chalton_Cl_Acc"),
            #lwd = 1.5, lty = 1) +
  #geom_line(aes(y = Ade_corrected, colour = "Adenta_MA_Acc"),
            #lwd = 1.5, lty = 1) +
  geom_line(aes(y = San_corrected, colour = "Santasi_Ksi"),
            lwd = 1.5, lty = 1) +
  geom_line(aes(y = Sua_corrected, colour = "Suame_MA_Ksi"),
            lwd = 1.5, lty = 1) +
  geom_line(aes(y = Sep_corrected, colour = "Sepe_Dote_Ksi"),
            lwd = 1.5, lty = 1) +
  geom_line(aes(y = Aso_corrected, colour = "Asokwa_Shell_Ksi"),
            lwd = 1.5, lty = 1) +
  geom_hline(aes(yintercept = 35, linetype = "GHANA STD (24-h)"),
             colour = "black", linewidth = 1) +
